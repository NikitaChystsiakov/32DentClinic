...
/*
 * Сама шторка мобильного меню. Вынесена из MobileMenuProvider в отдельный
 * модуль, потому что грузится лениво (React.lazy в провайдере): Sheet тянет
 * за собой Dialog из base-ui и floating-ui (~20 КБ gz), а нужны они только
 * после тапа по бургеру — на первом экране этот код не участвует.
 */
const NAV_ICONS = {
  implantaciya: Zap,
  uslugi: Stethoscope,
...
export function MobileMenuSheet() {
  ...
  const { isOpen, closeMenu, setOpen } = useMobileMenu()
